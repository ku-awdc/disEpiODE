...
```{r}
grid <- create_grid(landscape, patch_area = 0.001, celltype = "hexagon", rotate=0)
ggplot() +
...
```{r}
grid_sq <- create_grid(landscape, patch_area = 0.01, celltype = "square")
ggplot() +
  geom_sf(data=landscape) +
...
```{r}
grid_tri <- create_grid(landscape, patch_area = 0.002, celltype = "triangle", rotate=15)
ggplot() +
  geom_sf(data=landscape) +
...
tibble(PatchArea = 10^seq(0,-3,by=-0.05)) %>%
  ...
  lapply(function(x){
    ...
    grid <- create_grid(landscape, patch_area = x$PatchArea, celltype="triangle", rotate=0)
    overlap <- create_farm_overlap(grid, farms)
    init <- create_initial_state(grid, overlap, start_prev=0.5)
    beta_matrix <- create_transmission_matrix(grid, kernel, beta_baseline=0.05)
    taufun <- create_si_model(grid, beta_matrix, init, overlap, root="B")
