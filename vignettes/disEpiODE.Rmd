...
Once the grid and kernel are available, a transmission matrix can be generated. This will always be a symmetric matrix with dimension equal to the number of patches, and a diagnonal of one.

```{r}
beta_matrix <- create_transmission_matrix(grid, kernel)
nrow(grid)
dim(beta_matrix)
beta_matrix[1:5,1:5]
```
