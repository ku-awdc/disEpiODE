...
test_that("nested splits perfect tessellation", {
  ...
  for (n_cells in all_n_cells) {
    # current_grid <- st_make_grid(world_landscape,
    #                              square = TRUE,
    #                              n = n_cells)
    # current_grid <- st_make_grid(world_landscape,
    #                              square = TRUE,
    #                              cellsize = world_landscape/n_cells)
    # class(current_grid)
    curent_grid <- create_grid(world_landscape,
                               n = n_cells,
                               celltype = "square")
    expect_equal(sum(st_area(current_grid)),
                 st_area(world_landscape))

    flag <- current_grid %>% st_area() %>% zapsmall() %>% unique()
    flag
    expect_length(flag, 1)

  }
  ggplot() +
    geom_sf(data = current_grid, fill = NA) +
    theme_grid_plot()
