...
params1 <- tidyr::expand_grid(
  ...
  cellarea = c(NA, seq_cellarea(n = 200, min_cellarea = 1 / 3000, max_cellarea = 1)),
  n_cells = NA,
  # cellarea = NA,
...
seed_infection_df %>%
  ...
  {
    stopifnot(
      "seed infection mass must be the same across grids" = nrow(.) == 1,
...
progression_to_tau_plot <- tau_rstate %>%
  ...
  mutate(
    Farm = fct(Farm, levels = c("source", "middle", "target", "population")),
    Farm = fct_recode(
      Farm,
      `Farm A` = "source",
      `Farm B` = "middle",
...
ggsave(
  ...
  height = 4.5,
  # dpi=300,
  filename = glue("figures/{post_tag}_until_tau_plot_half-normal_hexagon.svg")
)
#'
#'
...
ggsave(
  device = svglite,
  width = 5, height = 3,
  filename = glue("figures/{post_tag}_landscape_plus_zones_plot.svg")
)
...
params1 %>%
  ...
  group_map(function(data, key) {
    ...
    tibble(
      distance = seq.default(0, 1, length.out = 200),
...
tau_plot_data <- tau_rstate %>%
  ...
  mutate(
    beta_mat = fct(beta_mat, kernel_levels),
...
tau_plot_data %>%
  ...
  mutate(beta_mat = beta_mat %>%
    fct_recode(
      Inverse = "inverse", Exponential = "exp",
  ...
  group_map(\(data, group_id) {
    ...
    data %>%
      ...
      guides(color = guide_legend(override.aes = list(linewidth = 2))) +
      labs(
        caption = glue("Kernel form: {beta_mat}"),
        y = expression(tau),
        x = "Set cellarea",
        color = NULL
      ) +
...
ggsave(
  ...
  scale = 3,
  filename = glue("figures/{post_tag}_tau_across_shapes.svg")
)
...
output_prevalence_at_tau_plot_df %>%
  ...
  group_map(\(data, kernel) {
    ...
    ggplot(data) +
      ...
      labs(
        caption = glue("Kernel form: {kernel_name}"),
        color = NULL,
        y = "Prevalence",
      ) +
      theme(
...
#' How does the configuration of `*_tau_state` look like
tau_rstate[[length(tau_rstate)]]$output$output_exp_tau_state %>%
  colnames()
...
source_seed_prevalence <-
  ...
  mutate(
    #' 1, means t = 0
    #' 1 skips time, 2 * .y skips Ss and Is,
    #' first prevalence is source
...
#'
if (source_seed_prevalence %>% count(celltype) %>%
  {
    any(.$n > 1)
  }) {
  source_seed_prevalence %>%
  ...
  source_seed_prevalence %>%
    ...
    geom_hline(
      data = . %>% distinct(seed_infection_proportion),
      aes(yintercept = seed_infection_proportion),
      linetype = "dashed"
    ) +
    geom_line(
  ...
  ggsave(
    ...
    height = 2.5,
    # dpi=300,
    filename = glue("figures/{post_tag}_prev_source_t0_all_grids.svg")
  )
}
