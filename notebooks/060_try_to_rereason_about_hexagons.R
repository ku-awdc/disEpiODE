...
cellarea <- world_scale**2 / 4
# cellarea <- world_scale**2 / 6
cellsize <- sqrt((2 * cellarea) / 3)
h <- cellsize * sqrt(3) / 2

#dx = cellsize[1]/sqrt(3)
#dy = sqrt(3) * dx/2

offset_x <- (world_scale %% cellsize) / 2
offset_y <- (world_scale %% h) / 2

hex_grid <- st_make_grid(world_landscape,
                         cellsize = c(cellsize, h),
                         offset = c(offset_x, offset_y),
                         square = FALSE) %>%
  st_union(
    st_make_grid(world_landscape -cellsize,
                 cellsize = c(cellsize, h),
                 offset = c(offset_x, offset_y)),
    square = FALSE)

plot(
  st_make_grid(world_landscape * 2,
               cellsize = c(cellsize, h))
)
...
plot(hex_grid)
plot(world_landscape, add = TRUE)

st_difference(world_landscape, hex_grid)
# st_union(by_feature = TRUE) %>%
plot()

hex_grid <- st_intersection(world_landscape, hex_grid) %>%
  identity()
# plot(st_geometry(hex_grid))
# plot(world_landscape, add = TRUE)
hex_grid %>% st_geometry_type() %>% unique()

stopifnot(
  isTRUE(all.equal(
    world_scale**2,
    hex_grid %>% st_union() %>% st_area()
  ))
)
