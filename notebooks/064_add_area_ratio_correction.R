...
params1 <- tidyr::expand_grid(
  ...
  cellarea = seq_cellarea(n = 100, min_cellarea = 1, max_cellarea = 10),
  n_cells = NA,
...
future_pmap(params1, .progress = TRUE,
            \(world_scale, beta_baseline, buffer_offset_percent, buffer_radius,
              ...
              # population_total <- world_area #unused
              ...
              for (beta_mat_name in beta_mat_list) {
                ...
                if (enable_area_ratio_correction) {
                  beta_mat = beta_mat %*% outer(1 / grid$area, grid$area)
                  # beta_mat = beta_mat %*% outer(grid$area, 1 / grid$area)
                  # beta_mat = outer(grid$area, 1 / grid$area) * beta_mat
                  # beta_mat = outer(1 / grid$area, grid$area) * beta_mat
                  # beta_mat = tcrossprod(1 / grid$area, beta_mat) %*% grid$area
